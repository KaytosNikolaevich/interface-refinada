...
create policy "User deletes own profile"
...
begin
  foreach t in array array['modules','lessons','exercises','stories'] loop
    ...
    execute format(
      'create policy "Catalog public read" on public.%I for select using (true);', t
    );
    execute format('drop policy if exists "Admin manages catalog" on public.%I;', t);
    execute format(
      'create policy "Admin manages catalog" on public.%I for all
        using (public.has_role(auth.uid(), ''admin''))
...
drop policy if exists "User sees own achievements" on public.user_achievements;
create policy "User sees own achievements"
  on public.user_achievements for select
  using (auth.uid() = user_id);

drop policy if exists "User unlocks achievement" on public.user_achievements;
create policy "User unlocks achievement"
  on public.user_achievements for insert
  with check (auth.uid() = user_id);

drop policy if exists "User removes own achievement" on public.user_achievements;
create policy "User removes own achievement"
...
drop policy if exists "User creates own progress" on public.user_progress;
create policy "User creates own progress"
  on public.user_progress for insert
  with check (auth.uid() = user_id);

drop policy if exists "User updates own progress" on public.user_progress;
create policy "User updates own progress"
  on public.user_progress for update
  using (auth.uid() = user_id);

drop policy if exists "User deletes own progress" on public.user_progress;
